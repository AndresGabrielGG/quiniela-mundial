...
export default function Dashboard() {
  ...
  if (!user) return <p className="min-h-screen bg-black text-white p-8 flex items-center justify-center font-black text-3xl tracking-widest">CARGANDO...</p>
    ...
    <main className="min-h-screen bg-black text-white p-4 md:p-8 font-sans">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-[#111] border-4 border-white p-6 mb-8 gap-6 rounded-none shadow-[8px_8px_0px_#ccff00]">
          ...
          <div className="flex flex-wrap gap-3 w-full md:w-auto font-black">
        ...
        {role === 'admin' && (
          <div className="mb-8 p-4 bg-white text-black border-4 border-black flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-[6px_6px_0px_#ff004d]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-black uppercase tracking-tighter text-2xl">Modo Admin</h2>
                <p className="text-sm font-bold text-gray-600">Acceso al Centro de Mando.</p>
              </div>
            </div>
            <button onClick={() => router.push('/admin')} className="w-full md:w-auto bg-black text-white font-black text-xl py-2 px-6 hover:bg-[#ff004d] transition-colors">
              ENTRAR
            </button>
          </div>
        )}
        ...
        <div className="flex justify-between items-end mb-6 border-b-4 border-white pb-3">
          <h3 className="text-3xl md:text-4xl font-black text-white tracking-wide">CALENDARIO OFICIAL</h3>
              ...
              <div key={match.id} className="bg-[#0a0a0a] border-4 border-[#222] hover:border-white transition-colors flex flex-col overflow-hidden relative">
                ...
                <div className="p-4 md:p-6 flex flex-col xl:flex-row items-center justify-between gap-6">
                  ...
                  <div className="w-full xl:w-48 shrink-0 flex flex-row xl:flex-col justify-between xl:justify-center items-center xl:items-start border-b-2 border-[#333] xl:border-none pb-3 xl:pb-0">
                    ...
                    {hasStarted && (
                      <div className={`mt-1 font-black tracking-widest text-sm px-3 py-1 inline-block border-2 border-white ${match.status === 'finished' ? 'bg-white text-black' : 'bg-[#ccff00] text-black animate-pulse'}`}>
                  ...
                  <div className="flex flex-col md:flex-row items-stretch md:items-center bg-[#111] border-2 border-white rounded-2xl md:rounded-full flex-1 max-w-2xl w-full justify-center shadow-lg overflow-hidden">
                    ...
                    {/* ZONA CENTRAL DE NÚMEROS (Fondo Claro) */}
                      ...
                      {!hasStarted ? (
                        <>
                          <input 
                            ...
                            className="w-12 h-12 md:w-14 md:h-14 text-center bg-black border-2 border-black font-black text-2xl md:text-3xl text-white focus:border-[#ff004d] outline-none transition-all rounded-md" 
                          />
                        ...
                        <div className="flex flex-col items-center">
                          <div className="flex items-center gap-3 text-black font-black text-4xl md:text-5xl">
                  ...
                  <div className="w-full xl:w-48 shrink-0 flex flex-col items-center xl:items-end justify-center mt-2 xl:mt-0 gap-2">
                    ...
                    {!hasStarted ? (
                      <button onClick={saveAllPredictions} className="w-full md:w-auto bg-white text-black font-black text-lg tracking-widest py-2 px-6 border-2 border-black hover:bg-[#ccff00] transition-colors mt-2 xl:mt-0">
                        GUARDAR
                      </button>
                    ) : (
                      <button 
                        onClick={() => setExpandedMatch(expandedMatch === match.id ? null : match.id)} 
                        className="w-full md:w-auto bg-[#222] text-white font-bold py-2 px-4 border-2 border-[#444] hover:bg-white hover:text-black transition-colors uppercase text-sm tracking-widest mt-2"
                      >
                        ...
                        return (
                          <div key={idx} className={`flex items-center gap-3 px-4 py-2 border-2 transition-all ${colorClass}`}>
                            {p.profiles?.avatar_url ? (
                              ...
                              <div className="w-8 h-8 bg-[#333] flex items-center justify-center font-black text-white border border-white/20 rounded-full">
                            ...
                            <span className={`font-black text-2xl tracking-widest ${textClass}`}>{p.pred_a} - {p.pred_b}</span>
        ...
        <button 
          ...
          className="md:hidden fixed bottom-6 right-6 bg-[#ccff00] text-black font-black text-2xl p-4 shadow-[4px_4px_0px_#fff] border-2 border-black z-50 active:translate-y-1 active:shadow-none transition-all"
        >
          💾
        </button>
